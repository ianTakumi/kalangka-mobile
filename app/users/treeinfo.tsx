...
export default function TreeInfoScreen() {
  ...
  // State for tree data (will be loaded from database)
  const [treeData, setTreeData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Get tree ID from params (from QR scan or navigation)
  const treeId =
    (params.treeData?.startsWith?.("{")
      ? JSON.parse(params.treeData).id
      : params.treeData) ||
    params.treeId ||
    params.id;
  ...
  // Load tree data from database using TreeService
  useEffect(() => {
    const loadTreeData = async () => {
      // If treeId is not provided, check if treeData was passed directly (backward compatibility)
      if (!treeId) {
      ...
      try {
        ...
        // Get tree by ID from local database
        const tree = await TreeService.getTreeById(treeId as string);

        if (tree) {
          setTreeData(tree);
          console.log("✅ Loaded tree from database:", tree);

          // Update initial position for map
          ...
          console.log("❌ Tree not found with ID:", treeId);
        }
      } catch (err: any) {
  ...
  // Create QR code data (only the tree ID, not the full object)
  ...
  const calculateDueDate = (wrappedDate: Date) => {
  ...
  // Load flower data for this tree
  useEffect(() => {
    const loadFlowerForTree = async () => {
      if (treeData?.id || treeData?._id) {
        try {
          // Import FlowerService dynamically to avoid circular dependencies
          ...
          console.log(`Loaded flowers for tree ${treeIdToUse}:`, flowers);
          if (flowers.length > 0) {
            setFlower(flowers[0]);
          } else {
            setFlower(null);
      ...
      <SafeAreaView className="flex-1 bg-gray-50">
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View className="px-4 -mt-8">
                ...
                <TouchableOpacity
                  onPress={() =>
                    router.push({
                      ...
                      params: { treeId: treeData.id || treeData },
                    })
                  }
            ...
            {/* QR Code Card - Now stores only the tree ID */}
            {treeData && (
              <View className="bg-white rounded-2xl shadow-lg p-6 mb-6">
