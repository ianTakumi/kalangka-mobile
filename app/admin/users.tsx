...
export default function Users() {
  ...
  const checkInitialSync = async () => {
    ...
    if (netState.isConnected && !hasSyncedFromServer) {
      console.log("📱 Online - Checking for server users...");
      try {
        const { needsSync, userCount } = await userService.checkAndSync();
        if (needsSync && userCount > 0) {
          console.log(`🔄 Server has ${userCount} users, auto-syncing...`);
        ...
        console.log("Could not check server status:", error);
      }
    }
  };
  ...
  const handleAutoSync = async () => {
    ...
    try {
      ...
      const { needsSync } = await userService.checkAndSync();
      if (needsSync) {
        await userService.syncUsersFromServer();
      }
      await loadUsers();
