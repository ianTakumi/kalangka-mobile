...
//   //   harvestThisWeek: HarvestService.getTotalWeightThisWeek(),
//   // });
...
export default function FarmerHomeScreen() {
  ...
  const [fruitsWithoutHarvest, setFruitsWithoutHarvest] = useState<Fruit[]>([]);
  const [loadingFruits, setLoadingFruits] = useState(false);
  const [loadingStats, setLoadingStats] = useState(true); // Add loading state for stats

  // Initialize with default values (not promises)
  const [userStats, setUserStats] = useState({
    totalTrees: 0,
    totalFlowers: 0,
    totalFruits: 0,
  ...
  const loadStats = async () => {
    try {
      ...
      const [
        ...
        HarvestService.getTotalWeightThisWeek(),
      ]);
  ...
  useEffect(() => {
    ...
    fetchFruitsWithoutHarvest(); // Load fruits
  ...
  const onRefresh = async () => {
    ...
    await Promise.all([
      ...
      fetchFruitsWithoutHarvest(), // Reload fruits
      fetchLocationAndWeather(), // Reload weather
    ]);
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
        ...
        <View className="flex-row justify-between items-center">
          ...
          >
            <RefreshCw
              size={18}
              color="#059669"
              className={`mr-1 ${weather.loading || loadingStats ? "animate-spin" : ""}`}
            />
            <Text className="text-sm text-emerald-600">Refresh</Text>
          </TouchableOpacity>
        </View>
      ...
      >
        ...
        <View className="px-6 pt-6">
          ...
          {loadingStats ? (
            // Show loading indicator while stats are loading
        ...
        {/* Bagged Fruits Pending Assignment Section - unchanged */}
        <View className="px-6 mb-6">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-2xl font-bold text-gray-900">
              Bagged Fruits Pending Assignment
          ...
          {loadingFruits ? (
            ...
            // ... rest of the fruits section (unchanged)
            <View className="bg-white rounded-2xl p-4 border border-orange-100">
